...
namespace JH.TC.DataExchange._103
{
    public partial class Map : BaseForm
    {
        ...
        //學生身分
        public static List<string> StatusList = new List<string>() { "原住民", "派外人員子女", "蒙藏生", "回國僑生", "港澳生", "退伍軍人", "境外優秀科學技術人才子女"};
        ...
        //身心障礙
        public static List<string> HandicappedList = new List<string>() { "智能障礙", "視覺障礙", "聽覺障礙", "語言障礙", "肢體障礙", "腦性麻痺", "身體病弱", "情緖行為障礙", "學習障礙", "多重障礙", "自閉症", "發展遲緩", "其他障礙" };

        //其他
        public static List<string> OtherList = new List<string>() { "低收入戶", "中低收入戶", "失業勞工子女", "非中華民國身分證號", "就近入學", "偏遠地區" };
        ...
        public Map()
        {
            ...
            foreach (var item in _TagConfigRecords)
            {
                if (item.Prefix != "")
                {
                    ...
                    StudentTag2.Items.Add(item.Prefix + ":" + item.Name);
                    StudentTag3.Items.Add(item.Prefix + ":" + item.Name);
                    StudentTag4.Items.Add(item.Prefix + ":" + item.Name);
                }
                else
                {
                    StudentTag.Items.Add(item.Name);
                    StudentTag2.Items.Add(item.Name);
                    StudentTag3.Items.Add(item.Name);
                    StudentTag4.Items.Add(item.Name);
                }
            }
