...
from 
    ...
    (
        select student.id
			,case when avg(cast( regexp_replace( xpath_string('<root>'||x1.score_info||'</root>','/root/Domains/Domain[@領域=''健康與體育'']/@成績'), '^$', '0') as float)) >= 60 then '達標準' else '未達標準' end as ""健康與體育""
			,case when avg(cast( regexp_replace( xpath_string('<root>'||x1.score_info||'</root>','/root/Domains/Domain[@領域=''藝術與人文'']/@成績'), '^$', '0') as float)) >= 60 then '達標準' else '未達標準' end as ""藝術與人文""
			,case when avg(cast( regexp_replace( xpath_string('<root>'||x1.score_info||'</root>','/root/Domains/Domain[@領域=''綜合活動'']/@成績'), '^$', '0') as float)) >= 60 then '達標準' else '未達標準' end as ""綜合活動""
		from 
            ...
            (
                ...
                FROM 
			        ...
			        left join sems_subj_score as x1 on student.id=x1.ref_student_id
			            and (
				            (''||x1.school_year=shistory.schoolyear3 and x1.semester= 1)--二上二下三上
            ...
            (
		        ...
		        from 
			        ...
			        and (
				        (''||x1.school_year=shistory.schoolyear3 and x1.semester= 1)--八上八下九上九下
				        or (''||x1.school_year=shistory.schoolyear4 and x1.semester= 2)
				        or (''||x1.school_year=shistory.schoolyear5 and x1.semester= 1)
